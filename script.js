...
function userLoadData() {
  ...
  let jsonUseString = JSON.stringify(user.users);
   localStorage.setItem("user-list", jsonUseString);
}
function getUserData() {
  let jsonUseString = localStorage.getItem("user-list");
  let userList = JSON.parse(jsonUseString);
  return userList;
}
...
function HeaderComponent(){
    ...
    optionMenu.setAttribute("class","d-flex justify-content-around align-items-center")

    let signIn = document.createElement('h6');

    signIn.innerText="Sign In"
    signIn.setAttribute("class","text-white")
    optionMenu.appendChild(signIn)

    let signUp = document.createElement('h6');

    signUp.innerText="Sign Up"
    signUp.setAttribute("class","text-white")
    optionMenu.appendChild(signUp)
...
let row = document.createElement("div");
row.setAttribute("class","row");
row.setAttribute("id","rowView")

let leftDiv = document.createElement("div");
leftDiv.setAttribute("class","col-md-6 d-flex flex-column align-items-center");
...
function ViewMoreComponent(product){
   ...
   viewMoreContainer.setAttribute("id","viewcomponent")
   ...
   imgFull.setAttribute("style","height : 350px ; width : 100%" )
   imgFull.src = product.thumbnail;

   

   leftDiv.appendChild(imgFull)

   // let imgSmall = document.createElement("img");
   // imgSmall.setAttribute("style","height : 100px ; width : 50%" )
   // imgSmall.src = product.thumbnail;

   // leftDiv.appendChild(imgSmall)

   let smallImgP = document.createElement("div")
    smallImgP.setAttribute("class" ,"d-flex ")
    leftDiv.appendChild(smallImgP)


    for (let i=0 ;i<product.images.length ;i++) {
       let smallImg = document.createElement("img");
        smallImg.setAttribute("style", "height:150px;");
        smallImg.src = product.images[i];
        smallImg.addEventListener("click",()=>{
            imgFull.src=product.images[i]
        })
        smallImgP.appendChild(smallImg);
    }
   ...
   productDetail.setAttribute("class","d-flex justify-content-between mt-4")
   let h4 = document.createElement("h4");
   h4.innerText = product.title

   let h2 = document.createElement("h2");
  
   h2.innerHTML = `<label class='text-danger' style ="text-decoration : line-through">${product.price} Rs.</label>`;
  

   rightDiv.appendChild(productDetail)
   productDetail.appendChild(h4)
   productDetail.appendChild(h2)

   let proContent = document.createElement("p");
   proContent.innerText = product.description

   rightDiv.appendChild(proContent)

    let categoryProduct = document.createElement("p");
   categoryProduct.innerHTML =`<b> Category Info : </b> ${product.category} <br>`
   rightDiv.appendChild(categoryProduct)

   let warrenty = document.createElement("p");
   warrenty.innerHTML =`<b> Warrenty Info : </b> ${product.warrantyInformation} <br>`
   rightDiv.appendChild(warrenty)

   let shipping = document.createElement("p");
   shipping.innerHTML =`<b> Shipping Info : </b> ${product.shippingInformation} <br>`
   rightDiv.appendChild(shipping)

   let returpolicy = document.createElement("p");
   returpolicy.innerHTML =`<b> Return Policy : </b> ${product.returnPolicy}<br> `
   rightDiv.appendChild(returpolicy)

   let disPer = product.discountPercentage;
   let price = product.price
   let discountPrice = price * disPer /100;

   let totalPrice = price - discountPrice;

   console.log(totalPrice.toString().slice(0,4))

   let discount = document.createElement("p");
   discount.innerHTML =`<b> Discount Price : </b> <span class ="text-success"> ${totalPrice.toString().slice(0,4)}  </span> <small> ${product.discountPercentage} % </small> <br> `
   rightDiv.appendChild(discount)

   let rating = document.createElement("p");
   rating.innerHTML =`<b> Rating  : </b> ${product.rating} <br>`
   rightDiv.appendChild(rating)

   let cart = document.createElement("div")
   cart.setAttribute("class","d-flex justify-content-around")
   rightDiv.appendChild(cart)

   let buyBtn = document.createElement("button");
   buyBtn.innerText = "Buy Now"
   buyBtn.setAttribute("style","width:100%")
   buyBtn.setAttribute("class","btn btn-warning mt-4 m-3 p-2")
...
function signIn(){
   let signInContainer = document.querySelector("#main");
   ...
   signInForm.setAttribute("id", "sign-container");
   ...
   <button  id="loginbtn" class="btn btn-warning w-100 mt-3">Login</button>
   <p id ="error-msg" class="text-danger mt-2 text-center"></p>
   </div>
   </div>
   `;
   ...
   document.getElementById("loginbtn").addEventListener("click", function(){
      ...
      let checkUserCred = userList.find(function(user){
